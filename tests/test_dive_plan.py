...
def test_depth_to_P_amb():
    ...
    for depth, expected_pressure in depths_and_pressures.items():
        ...
        assert P_amb == expected_pressure, f"Expected {expected_pressure} bar at {depth}m, but got {P_amb}."
...
def test_best_gas_mix():
    ...
    for depth in depths:
        ...
        for gas in gases:
            assert gas.O2 >= 0, f"Invalid O2 concentration ({gas.O2}) in gas mix at {depth}m."
...
def test_make_best_mix():
    ...
    for depth in depths:
        ...
        assert best_gas.O2 >= 0, f"Invalid O2 concentration ({best_gas.O2}) in best gas mix at {depth}m."
        assert 0 <= best_gas.O2 <= 1, f"O2 concentration out of range (0-1) for best gas mix at {depth}m."
